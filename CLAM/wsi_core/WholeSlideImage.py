...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
        ...
        self.wsi = opensdpc.OpenSdpc(path)  # 此处为处理特殊的sdpc格式切片
    ...
    #     for idx, cont in enumerate(self.contours_tissue):
    #         if (idx + 1) % fp_chunk_size == 0:
    #             print('Processing contour {}/{}'.format(idx + 1, n_contours))
    ...
    #         if len(asset_dict) > 0 and "coords" in asset_dict:
    ...
    #                     'y': coord[1]
    #                 }
    #                 all_attrs.append(patch_attr)
    #                 patch_count += 1

    #     if all_coords and all_patches:
    #         all_coords_array = np.array(all_coords)
    #         all_patches_array = np.array(all_patches, dtype=np.uint8)

    #         final_asset_dict = {
    #             'coords': all_coords_array,
    #             'patches': all_patches_array,
    #             # 这里也可以存 all_attrs（如果 save_hdf5 支持）
    #         }
    ...
    #             'wsi_shape': tuple(self.level_dim[patch_level]),
    #             'contour_count': n_contours
    #         }
            
    #         save_hdf5(save_path_hdf5, final_asset_dict, final_attr_dict, mode='w')
    #         print(f"Saved {patch_count} patches to {save_path_hdf5}")
    #     else:
    ...
    def process_contours(self, save_path, patch_level=0, patch_size=256, step_size=256, **kwargs):
        ...
        for idx, cont in enumerate(self.contours_tissue):
            ...
            if len(asset_dict) > 0:
                # 确保写入前维度一致
                if "coords" in asset_dict:
                    coords = asset_dict["coords"]
                    dset_shape = coords.shape[1]

                    # 如果 dataset 里只接受 2 维，而 coords 多了一列
                    if dset_shape > 2:
                        coords = coords[:, :2]  # 丢掉多余的列
