...
def resnet50_baseline(pretrained=False):
    ...
    if pretrained:
        print("Loading pre-trained weights from ImageNet...")
...
def load_pretrained_weights(model, name):
    ...
    print(f"Downloading pre-trained weights for {name}...")
    print(f"URL: {model_urls[name]}")
    
    # 加载预训练权重
    pretrained_dict = model_zoo.load_url(model_urls[name])
    
    print("Weights downloaded, now loading into model...")
    
    # 加载权重到模型（非严格模式，允许不匹配的层）
    ...
    if missing_keys:
        print(f"Warning: {len(missing_keys)} parameters were not loaded (this may be expected for modified architectures):")
        for key in missing_keys[:5]:  # 只显示前5个未加载的键
            print(f"  - {key}")
