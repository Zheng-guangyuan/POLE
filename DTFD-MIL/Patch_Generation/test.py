...
save_folder_dir = 'D:\AAAAny\TEMP\patches4DTFD_1021'
...
def visualize_mask_and_bboxes(tslide, mask, boundingBoxes, mask_level, save_path):
    """
    可视化mask和boundingBoxes在病理切片上的效果
    Args:
        tslide: sdpc.Sdpc对象
        mask: 二值mask (0/255)
    ...
    # 读取低倍图像
    slide_img = np.array(tslide.read_region((0, 0), mask_level, tslide.level_dimensions[mask_level]))
    # 叠加mask（半透明红色）
    mask_rgb = np.zeros_like(slide_img)
    mask_rgb[..., 0] = mask  # 红色通道
    overlay = cv2.addWeighted(slide_img, 0.7, mask_rgb, 0.3, 0)

    # 画boundingBoxes
    for i, (x, y, w, h) in enumerate(boundingBoxes):
        cv2.rectangle(overlay, (x, y), (x + w - 1, y + h - 1), (0, 255, 0), 4)
        cv2.putText(overlay, f'{i}', (x, max(y-6,0)), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0,255,0), 3)

    # 保存
    Image.fromarray(overlay.astype(np.uint8)).save(save_path)
    print(f'可视化结果已保存: {save_path}')

# 测试入口（可放在__main__下，指定一个切片名）
if __name__ == "__main__":
    test_slide_name = "B2342925-1#13_5" 
    test_slide_path = 'E:\SYSUXY\/' + test_slide_name + ".sdpc" 
    tslide = sdpc.Sdpc(test_slide_path)
    mask_level = min(mask_dimension_level, tslide.level_count - 1)

    mask, boundingBoxes = get_roi_bounds(tslide, mask_level=mask_level)

    Thread_PatchFromSlides((test_slide_path, test_slide_name, os.path.join("D:\AAAAny\Workspace\POLE\TMP", test_slide_name)))

    # 可视化
    visualize_mask_and_bboxes(tslide, mask, boundingBoxes, mask_level, f"D:\AAAAny\Workspace\POLE\TMP\{test_slide_name}_mask_bbox_vis.png")