...
class CamelData(data.Dataset):
    def __init__(self, dataset_cfg=None,
        ...
        self.csv_dir = self.dataset_cfg.label_dir + 'splits.csv'
    ...
    def __getitem__(self, idx):
		...
		# build path robustly
        pkl_path = Path(self.feature_dir) / (slide_id + '.pkl')
        with open(pkl_path, 'rb') as f:
            data = pickle.load(f)

        # extract features and convert to tensor
        features_list = [item['feature'] for item in data]
        multi = np.stack(features_list)
        features = torch.from_numpy(multi).float()
